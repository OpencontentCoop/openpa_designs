...
{if and( $content_object.can_create, $is_container )}
    <div id="ezwt-creataction" class="ezwt-actiongroup">
        {def $class_comunicato=fetch( 'content', 'class', hash( 'class_id', 'comunicato' ) )}
        <a href="#" data-id="{$class_comunicato.id}" class="btn btn-info" id="create-comunicato" title="Crea comunicato"><i class="fa fa-file-text-o" aria-hidden="true"></i> <span class="hidden-sm">Crea comunicato</span></a>
        <script>{literal}
            $(document).ready(function(){
                $("#create-comunicato").on( 'click', function(e){
                    e.preventDefault();
                    var button = $(this);
                    $("#ezwt-create").val(button.data('id'));
                    console.log($("#ezwt-create").val());
                    //button.parents('form').submit();
                    $("#InputCreate").trigger('click');
                });
            });
            {/literal}</script>
    </div>
...
{def $can_create_class_list = ezcreateclasslistgroups( $content_object.can_create_class_list )}  
  ...
  <input class="ezwt-input-image" id="InputCreate" type="image" src={"websitetoolbar/ezwt-icon-create.png"|ezimage} name="NewButton" title="{'Create here'|i18n('design/standard/parts/website_toolbar')}" />
</div>
{/if}
