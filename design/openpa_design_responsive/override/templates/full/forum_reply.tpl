<div class="content-view-full">
    <div class="class-forum-reply">
        ...
        <div class="attribute-header">
            <h1>{"Message preview"|i18n("design/openroom/full/forum_reply")}</h1>
        </div>
        ...
        <tr>
            <th class="author">
            {"Author"|i18n("design/openroom/full/forum_reply")}
            </th>
            <th class="message">
            {"Topic"|i18n("design/openroom/full/forum_reply")}
            </th>
        </tr>
        <tr class="bglightforum">
            ...
            {def $owner=$node.object.owner
                ...
                {if is_set( $owner_map.location )}
                    <p>{"Location"|i18n("design/openroom/full/forum_reply")}:{$owner_map.location.content|wash}</p>
                {/if}
                <p>
                {def $owner_id=$node.object.owner.id}
                    {foreach $node.object.author_array as $author}
                        {if eq($owner_id,$author.contentobject_id)|not()}
                            {"Moderated by"|i18n("design/openroom/full/forum_reply")}: {$author.contentobject.name|wash}
                         {/if}
                     {/foreach}
                </p>
                ...
                <input class="button forum-edit-reply" type="submit" name="EditButton" value="{'Edit'|i18n('design/openroom/full/forum_reply')}" />
