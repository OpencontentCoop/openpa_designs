    ...
    <div class="panel panel-editor-tool">
        ...
        <div class="panel-body">
            <ul class="list-unstyled">
                ...
                <li><strong>Collocazioni:</strong> <ul>{foreach $node.object.assigned_nodes as $item}<li><a href={$item.url_alias|ezurl()}>{$item.path_with_names}</a></li>{/foreach}</ul></li>
                {/if}
            ...
            {if and( $logged_user.is_logged_in, or( $node.class_identifier|eq( 'pagina_sito' ), $node.class_identifier|eq( 'folder' ) ) )}
                ...
                {if $notification_access}
                    <form class="clearfix" method="post" action={"/content/action/"|ezurl}>			   
                        <input class="pull-right btn btn-info btn-xs" type="submit" name="ActionAddToNotification" value="{'Keep me updated'|i18n('design/retecivica/full/folder')}" />
