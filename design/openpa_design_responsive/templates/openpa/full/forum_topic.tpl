     ...
     next_topic=fetch_alias( subtree, hash( parent_node_id, $node.parent_node_id,
        ...
        <ul class="pagination">
            {section show=$previous_topic}                
                <li><a href={$previous_topic[0].url_alias|ezurl}>&laquo; {'Previous topic'|i18n( 'design/openroom/full/forum_topic' )}</a></li>
            {section-else}
                <li class="disabled"><a href="#">&laquo; {'Previous topic'|i18n( 'design/openroom/full/forum_topic' )}</a></li>
            {/section}
            ...
            {section show=$next_topic}
                <li class="active"><a href={$next_topic[0].url_alias|ezurl} title="{$next_topic[0].name|wash}">{'Next topic'|i18n( 'design/openroom/full/forum_topic' )}</a> &raquo;</li>
            {section-else}
                <li class="disabled">
                    <a href="#">{'Next topic'|i18n( 'design/openroom/full/forum_topic' )}&nbsp;&raquo;</a>
                </li>
            {/section}
        </ul>
        </div>
        ...
        <form method="post" action={"content/action/"|ezurl}>
            <input class="btn btn-md btn-success forum-new-reply" type="submit" name="NewButton" value="{'New reply'|i18n( 'design/openroom/full/forum_topic' )}" />
            <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
            <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
            {if $notification_access}
                <input class="btn btn-md btn-info forum-keep-me-updated" type="submit" name="ActionAddToNotification" value="{'Keep me updated'|i18n( 'design/openroom/full/forum_topic' )}" />
            {/if}
        ...
        {section-else}
           <div class="aert alert-danger"><p>
            {"You need to be logged in to get access to the forums. You can do so %login_link_start%here%login_link_end%"|i18n( "design/openroom/full/forum_topic",,
        ...
        {/section}
            ...
            <tr>
                <th class="author">
                    {"Author"|i18n("design/openroom/full/forum_topic")}
                </th>
                <th class="message">
                    {"Message"|i18n("design/openroom/full/forum_topic")}
                 </th>
            </tr>
            ...
            <tr class="bglight">
               ...
               {let owner=$node.object.owner owner_map=$owner.data_map}
                   ...
                   {section show=is_set( $owner_map.location )}
                       <p>{"Location"|i18n( "design/openroom/full/forum_topic" )}: {$owner_map.location.content|wash}</p>
                   {/section}
                   <p>
                      {let owner_id=$node.object.owner_id}
                          {section var=author loop=$node.object.author_array}
                              {section show=eq($owner_id,$author.contentobject_id)|not()}
                                  {"Moderated by"|i18n( "design/openroom/full/forum_topic" )}: {$author.contentobject.name|wash}
                              {/section}
                          {/section}
                      {/let}
                   </p>
                  ...
                  {section show=$node.object.can_edit}
                      <form method="post" action={"content/action/"|ezurl}>
                          ...
                          <input class="btn btn-xs btn-warning forum-account-edit" type="submit" name="EditButton" value="{'Edit'|i18n('design/openroom/full/forum_topic')}" />
                  ...
                  {section show=$node.object.can_remove}
                      <form method="post" action={"content/action/"|ezurl}>
                          <input type="hidden" name="ContentObjectID" value="{$node.object.id}" />
                          <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
                          <input class="btn btn-xs btn-danger" type="submit" name="ActionRemove" value="{'Remove'|i18n( 'design/openroom/full/forum_topic' )}" title="{'Remove this item.'|i18n( 'design/openroom/full/forum_topic' )}" />                      </form>
                  {/section}
                ...
                <tr class="{$reply.sequence}">
                    ...
                    {let owner=$reply.object.owner owner_map=$owner.data_map}
                        ...
                        {case match=1}
                            ...
                            <input class="btn btn-xs btn-warning" type="submit" name="EditButton" value="{'Edit'|i18n('design/openroom/full/forum_topic')}" />
                            <input type="hidden" name="ContentObjectLanguageCode" value="{ezini( 'RegionalSettings', 'ContentObjectLocale', 'site.ini')}" />
                            </form>
                        {/case}
                        {case match=0}
                        ...
                        {section show=$node.object.can_remove}
                            <form method="post" action={"content/action/"|ezurl}>
                                ...
                                <input class="btn btn-xs btn-danger" type="submit" name="ActionRemove" value="{'Remove'|i18n( 'design/openroom/full/forum_topic' )}" title="{'Remove this item.'|i18n( 'design/openroom/full/forum_topic' )}" />
                                </form>
                        {/section}
