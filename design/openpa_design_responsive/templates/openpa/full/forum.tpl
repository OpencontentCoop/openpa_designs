{def $page_limit = 20
    ...
    {if $node.object.can_create}
        ...
        <form method="post" action={"content/action/"|ezurl}>
            <input class="btn btn-md btn-success forum-new-topic" type="submit" name="NewButton" value="{'New topic'|i18n( 'design/openroom/full/forum' )}" />
            <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
            <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
            <input type="hidden" name="ContentLanguageCode" value="{ezini( 'RegionalSettings', 'ContentObjectLocale', 'site.ini')}" />
            {if $notification_access }
                <input class="btn btn-md btn-info forum-keep-me-updated" type="submit" name="ActionAddToNotification" value="{'Keep me updated'|i18n( 'design/openroom/full/forum' )}" />
            {/if}
    ...
    {else}
        <div class="alert alert-warning>
            <p>
            {"You need to be logged in to get access to the forums. You can do so %login_link_start%here%login_link_end%"|i18n( "design/openroom/full/forum",, hash( '%login_link_start%', concat( '<a href=', '/user/login/'|ezurl, '>' ), '%login_link_end%', '</a>' ) )}
            </p>
        </div>
    {/if}
    {/if}
    ...
    <div class="table-responsive">
        ...
        <tr>
            <th class="topic">
                {"Topic"|i18n( "design/openroom/full/forum" )}
            </th>
            <th class="replies">
                {"Replies"|i18n( "design/openroom/full/forum" )}
            </th>
            <th class="author">
                {"Author"|i18n( "design/openroom/full/forum" )}
            </th>
            <th class="lastreply">
                {"Last reply"|i18n( "design/openroom/full/forum" )}
            </th>
        </tr>
            ...
            <tr class="{$topic.sequence}">
                <td class="topic{section show=$topic.object.data_map.sticky.content} danger{/section}">
                    ...
                    {section show=$topic_reply_count|gt( sub( 20, 1 ) )}
                        <p>
                        {'Pages'|i18n( 'design/openroom/full/forum' )}:
