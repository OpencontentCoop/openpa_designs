jQuery(function( $ )
{
    ...
    {
        e.preventDefault();
        var box = $(this).parents( '.searchbox' ), text = box.find('input.ezobject-relation-search-text');
        if ( text.val() )
        {
                    ...
                    if ( data.content.SearchResultCount > 0 )
                    {
                        ...
                        for ( var i = 0, l = arr.length; i < l; i++ )
                        {
                            ...
                            aElem.bind( 'click', { table: box.find('table'),
                                ...
                                ezajaxrelationsSearchAddObject( this, $(e.currentTarget).parents('.searchbox').find('table'), e.data.id, e.data.name, e.data.className, e.data.sectionName, e.data.publishedTxt );
                            } );
