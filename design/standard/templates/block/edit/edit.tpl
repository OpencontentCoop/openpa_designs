...
<label>Colore</label>
<input type="radio" id="block-custom_attribute-{$block_id}-{19771205}" class="block-control" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][color]" {if is_set($block.custom_attributes[color])|not()}checked="checked"{/if} value="" /> 
<div style="vertical-align: text-top; margin-right: 5px; width: 15px; height: 15px; display: inline-block; border: 1px solid #ccc"></div>
{foreach $colors as $color}
<input type="radio" id="block-custom_attribute-{$block_id}-{19771205}" class="block-control" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][color]" {if $block.custom_attributes[color]|eq($color)}checked="checked"{/if} value="{$color}" />
<div style="vertical-align: text-top; margin-right: 5px; width: 15px; height: 15px; display: inline-block; background-color: #{$color}"></div>
{/foreach}
{/if}
