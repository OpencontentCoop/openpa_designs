    ...
    <div id="footer-design">
        {include uri='design:menu/full_bottom.tpl'}
        {*
        ...
        *}
        ...
        <div class="info">        
            {def $contacts = fetch( 'content', 'node', hash( 'node_id', openpaini( 'LinkSpeciali', 'NodoContattaci ', 1032 ) ) )}
            {if and( $contacts, is_set( $contacts.data_map.abstract ), $contacts.data_map.abstract.has_content)}
            {attribute_view_gui attribute=$contacts.data_map.abstract}
            {/if}
        </div>
