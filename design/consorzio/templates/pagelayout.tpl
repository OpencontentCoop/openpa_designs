...
{def $logo = 'images/consorzio/CCT_logo.png'|ezdesign(no)
     $slug = 'Portale a servizio dei Comuni e delle Comunit&agrave; di Valle della Provincia di Trento'}
...
<div id="page" class="{$pagestyle} {$main_style}">
    ...
    <div id="header" class="float-break width-layout">        
        ...
        <div id="logo">
            <h1><a style="background:url({$logo}) no-repeat center" href={"/"|ezurl} title="{ezini('SiteSettings','SiteName')}">{ezini('SiteSettings','SiteName')}</a></h1>
        </div>  
