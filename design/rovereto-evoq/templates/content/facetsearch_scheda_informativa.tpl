...
{/if}
{* FORZO la disattivazione della date *}
{set $useDateFilter = false()}
    ...
    {foreach $selected_class_attributes as $attribute}
        {if and( $attribute.is_searchable, $attribute.identifier|ne('errors'), $attributi_da_escludere_dalla_ricerca|contains( $attribute.identifier )|not() )}
            ...
            {foreach $facets as $facet}
                {if $facet.field|eq( concat( solr_meta_subfield($attribute.identifier,'id') )}
                    {set $do = false()}
                    {break}
                {/if}
            {/foreach}
...
{* controllo i view_parameters per il sort *}
...
<div class="class-{$node.object.class_identifier} row">
	<div class="col-md-12">
        ...
        <div class="row">
            <div class="col-md-8" id="children">
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <h2>
                                    ...
                                    {$facets.$key.name|explode( '_' )|implode( ' ' )|wash()}
                                </h2>
                            </div>
                                    ...
                                    <li {if $cssstyle|count()}class="{$cssstyle|implode( ' ' )}"{/if}>                                
                                        {if $current}
                                            ...
                                            {foreach $view_parameters as $key2 => $value}
                                                ...
                                                {/if}
                                            {/foreach}
                                            <a class="helper" href="{concat( $node.url_alias, $uristring )|ezurl(no)}" title="Rimuovi filtro">
                            ...
                            {if and( is_set( $view_parameters.dateFilter ), $view_parameters.dateFilter|gt( 0 ), $view_parameters.dateFilter|lt( 6 ) )}
                                <li class="current">                
                                    {set $dateString = $dateString|explode( concat( '/(dateFilter)/', $view_parameters.dateFilter ) )|implode( '' )
                                         $dateStyle = 'current'}
                                    <a class="helper" href="{concat( $node.url_alias, $dateString )|ezurl(no)}" title="Rimuovi filtro">
                                        <span class="glyphicon glyphicon-remove"></span>
                                    </a>
