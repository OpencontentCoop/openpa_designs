...
<div class="topmenu-design{if $custom_menu} custom{/if}">
    ...
    {def $root_node=fetch( 'content', 'node', hash( 'node_id', $pagedata.root_node) )}
        ...
        {if $custom_menu|not()}
            {def $top_menu_items=fetch( 'content', 'list', hash( 'parent_node_id', $root_node.node_id,
            ...
            {* aggiunge il nodo Home all inizio *}
            {set $top_menu_items= $top_menu_items|prepend($root_node)}
		...
		{if $top_menu_items_count}
		   {foreach $top_menu_items as $key => $item}
            ...
            {if eq($item.node_id, $root_node.node_id)}
              <div><a title="{$item.name|wash()}" href={$item.url_alias|ezurl()}><span>Home</span></a></div>
            {else}
            <div>
