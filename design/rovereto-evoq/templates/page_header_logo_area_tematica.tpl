...
<div class="row">
    <div class="col-md-6">
        <h1 id="logo_area_tematica">            
            {if and( is_set( is_area_tematica().data_map.logo ), is_area_tematica().data_map.logo.has_content )}
                {attribute_view_gui attribute=is_area_tematica().data_map.logo image_class=medium href=is_area_tematica().url_alias|ezurl()}
            {else}
            <a href={is_area_tematica().url_alias|ezurl} title="{is_area_tematica().name|wash()}">                
                {is_area_tematica().name|wash()}
            </a>
            {/if}
        </h1>
    </div>
    <div class="col-md-6">
        <div class="row">            
            ...
            {/if}
            <div class="col-xs-12">
              {if and( is_set( is_area_tematica().data_map.show_search ), is_area_tematica().data_map.show_search.data_int|eq(1) )}
