...
<div class="class-{$node.object.class_identifier} row">
	<div class="col-md-12">
        ...
        {if or( and( is_set($node.data_map.contatti), $node.data_map.contatti.has_content ),
                and( is_set($node.data_map.link_esterno), $node.data_map.link_esterno.has_content ),
                and( is_set($node.data_map.geo), $node.data_map.geo.has_content )
              )}
        <div class="attributi-base">
        {include name=attribute_group
                 node=$node
                 identifiers=array( 'contatti', 'link_esterno', 'geo' )
