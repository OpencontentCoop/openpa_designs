...
{if is_area_tematica()}
    ...
    {if is_set( is_area_tematica().data_map.top_menu )}
        {if is_area_tematica().data_map.top_menu.content|ne(1)}
            {set $top_menu = false()}
        {/if}
    {/if}

    {if is_set( is_area_tematica().data_map.left_menu )}
        {if or( is_area_tematica().data_map.left_menu.content|ne(1), $node.node_id|eq( is_area_tematica().node_id ) )}
            {set $left_menu = false()}
        {/if}
    {/if}

    {if is_set( is_area_tematica().data_map.extra_menu )}
        {if is_area_tematica().data_map.extra_menu.content|ne(1)}
            {set $extra_info = false()}
        {/if}
    {/if}

    {if is_set( is_area_tematica().data_map.show_path )}
        {if is_area_tematica().data_map.show_path.content|ne(1)}
            {set $show_path = false()}
        {/if}
    {/if}
...
{def $background = section_image()
     ...
     $background_css = ''}
{if and( is_area_tematica(), $background_ignore_home|ne(false())  )}
    {set $background_css = concat('style="background: url(', $background_ignore_home.background.url|ezroot(no), ') no-repeat top center;"')}
{elseif and( is_area_tematica(), is_area_tematica().data_map.cover.has_content )}
    {set $background_css = concat('style="background: url(', is_area_tematica().data_map.cover.content.background.url|ezroot(no), ') no-repeat top center;"')}
...
{if $homepage.node_id|eq($node.node_id)}
    {ezpagedata_set('page_css_class', 'home')}
...
{elseif is_section(  'citta' )}
    <div id="content" class="container citta">
        ...
        {include uri='design:menu/top.tpl' start_node=$citta pagedata=$pagedata}

        {set $pagedata = $pagedata|merge(hash(
            'show_path',  true()
        ))}

        {if $node.node_id|ne($sections.citta)}
            {include uri='design:parts/path.tpl' pagedata=$pagedata current_node_id=$node.node_id}
        {/if}
{else}
