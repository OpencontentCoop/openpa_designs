...
<div class="content-view-full class-{$node.class_identifier} row">
    ...
    {def $search_materie = fetch( ezfind, search, hash( sort_by, hash( 'name', 'asc' ), limit, 200, class_id, array( 'materia' ), filter, array( concat( solr_meta_subfield('referente_politico','id'),':', $current_object_id ) ) ) )}
    ...
    {def $search_desginazioni = fetch( ezfind, search, hash( class_id, array( 'designazione' ), filter, array( concat( solr_meta_subfield('persona_designata','id'),':', $current_object_id ) ) ) )}
    ...
    <div class="content-related">
      ...
      <ul class="list-unstyled">
        {def $search_osservazioni_c = fetch( ezfind, search, hash( class_id, array( 'seduta' ), limitation, array(), filter, array( concat( solr_meta_subfield('presenti','id'),':', $current_object_id ), concat(solr_subfield('organo','name','string'),':Consiglio' ) ) )}
        <li>{$search_osservazioni_c.SearchCount} sedute di Consiglio</li>
        {def $search_osservazioni_g = fetch( ezfind, search, hash( class_id, array( 'seduta' ), limitation, array(), filter, array( concat( solr_meta_subfield('presenti','id'),':', $current_object_id ), concat(solr_subfield('organo','name','string'),':Giunta' ) ) ) )}
        ...
        {def $search_audizioni = fetch( ezfind, search, hash( class_id, array( 'audizione' ), limitation, array(), filter, array( concat( solr_meta_subfield('referente_politico','id'),':', $current_object_id ) ) ) )}
